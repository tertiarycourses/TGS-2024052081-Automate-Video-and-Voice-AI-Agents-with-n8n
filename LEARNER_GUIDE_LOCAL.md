...
> **This is the LOCAL build** — n8n in Docker, models on your own machine (Ollama `gemma4` +
> `nomic-embed-text`), webhooks on `http://localhost:5678`. Labs live in [`labs_local_n8n/`](labs_local_n8n/).
> For the hosted build (no Docker, OpenAI models) see [`LEARNER_GUIDE_CLOUD.md`](LEARNER_GUIDE_CLOUD.md).
>
> ⚠️ **Sections on Lab 4 (voice booking) and Lab 5 (FAQ voice agent) are OUT OF DATE.** Lab 4 is now
> **ElevenLabs** (was Retell) and Lab 5 is now **MediRefill**, a prescription-refill pharmacy (was
> HomeMart). Until this guide is regenerated, follow
> [`labs_local_n8n/lab4/README.md`](labs_local_n8n/lab4/README.md) and
> [`labs_local_n8n/lab5/README.md`](labs_local_n8n/lab5/README.md), which are current.
...
|---|---|---|
| Serve a lab website | `cd labs_local_n8n/lab4/website`<br>`python3 -m http.server 8090` | `cd lab4\website`<br>`python -m http.server 8090` |
| One-click launcher | double-click `start.command` | double-click `start.bat` |
| Path separator | `labs_local_n8n/lab4/website` (forward slash) | `lab4\website` (backslash) |
...
3. Start the n8n stack from `labs_local_n8n/lab0/docker-compose.yml` with `docker compose up -d`.
...
![The seven course workflows imported into local n8n.](courseware/screenshots/n8n-workflow-list.png)
...
1. Import `labs_local_n8n/lab1/ai-agent-ollama.json` into n8n.
...
![Lab 1 - AI Agent (Ollama): chat trigger, AI Agent and the local Ollama model.](courseware/screenshots/lab1-ai-agent-ollama.png)
...
1. Import `labs_local_n8n/lab2/rag-flow.json` into n8n.
2. Select the Ollama credential in both chat and embedding nodes.
3. Open `labs_local_n8n/lab2/index.html` in a browser.
4. Upload `labs_local_n8n/lab2/it-faq.pdf` through the page and confirm the insert path runs.
...
![Lab 2 - RAG IT Support Chatbot: ingestion, embeddings and the vector store.](courseware/screenshots/lab2-rag-flow.png)

*Lab 2 - RAG IT Support Chatbot: ingestion, embeddings and the vector store.*

![The brochure uploader page: the learner pastes their OWN n8n webhook URL.](courseware/screenshots/lab2-brochure-uploader.png)
...
1. Import `labs_local_n8n/lab3/CX Agent with RAG.json`.
2. Activate the workflow and confirm the brochure upload webhook URL.
3. Open `labs_local_n8n/lab3/upload-brochures.html` and upload all 20 brochure text files.
4. Open `labs_local_n8n/lab3/website/index.html` and start a customer chat.
...
![Lab 3 - CX Agent with RAG: the agent plus its retrieval tool.](courseware/screenshots/lab3-cx-agent-rag.png)

*Lab 3 - CX Agent with RAG: the agent plus its retrieval tool.*

![Lab 3 - Cook & Bake Academy site: the customer-facing front end.](courseware/screenshots/lab3-website-home.png)

*Lab 3 - Cook & Bake Academy site: the customer-facing front end.*

![Lab 3 - the chat widget's gear: each learner points it at their own n8n webhook.](courseware/screenshots/lab3-chat-webhook-settings.png)
...
![The Retell agent: prompt, Functions (the n8n tool webhooks) and Knowledge Base.](courseware/screenshots/retell-agent.png)
...
![Functions -> Custom Function: this is where the Retell -> n8n webhook URL goes.](courseware/screenshots/retell-functions.png)
...
![http://127.0.0.1:4040 - ngrok's own status page. URL = your public address; Addr = the local n8n it forwards to.](courseware/screenshots/ngrok-status.png)
...
1. Import `labs_local_n8n/lab4/retell-web-call-flow.json`.
...
5. Serve the site: double-click `labs_local_n8n/lab4/website/start.command` (macOS) or `start.bat` (Windows). Or by hand: `cd labs_local_n8n/lab4/website` then `python3 -m http.server 8090` on macOS / `python -m http.server 8090` on Windows. Open `http://localhost:8090`.
...
![Lab 4 - Retell Web Call Trigger: webhook -> Retell create-web-call -> access token.](courseware/screenshots/lab4-retell-web-call.png)

*Lab 4 - Retell Web Call Trigger: webhook -> Retell create-web-call -> access token.*

![Lab 4 - GG Hair Salon site: the Book by Voice call to action.](courseware/screenshots/lab4-website-home.png)

*Lab 4 - GG Hair Salon site: the Book by Voice call to action.*

![Lab 4 - Settings: the learner's own n8n webhook URL and Retell agent ID. Nothing is hardcoded.](courseware/screenshots/lab4-webhook-settings.png)
...
1. Open `labs_local_n8n/lab4/knowledge-base/gg-hair-salon-handbook.pdf` and note three facts that appear ONLY in the PDF and nowhere in the agent prompt: the 12-hour cancellation rule, the $30 colour deposit, and the 15-minute late-arrival grace period. These are your test targets. (Edit the content with `build_kb_pdf.py` if you want.)
...
![Knowledge Base -> Add: name it, then Upload Files and select the salon handbook PDF.](courseware/screenshots/retell-knowledge-base.png)

*Knowledge Base -> Add: name it, then Upload Files and select the salon handbook PDF.*

![The handbook attached to the agent. Publish the agent or the live call keeps the old config.](courseware/screenshots/retell-kb-attached.png)
...
![Select Voice: cloning lives HERE, in the agent's voice selector - not in the sidebar.](courseware/screenshots/retell-voice-picker.png)

*Select Voice: cloning lives HERE, in the agent's voice selector - not in the sidebar.*

![Add Voice Clone: name, upload an audio clip under 10 MB, and tick the consent box.](courseware/screenshots/retell-voice-clone.png)
...
2. Import `labs_local_n8n/lab4/retell-booking-tools-flow.json`. It has two webhook paths: `check-availability` and `book-appointment`.
...
![Lab 4.7 - the booking tools: check-availability and book-appointment, both wired to Google Calendar.](courseware/screenshots/lab4-booking-tools.png)
...
8. In Vapi, create an assistant named `Ava - HomeMart FAQ`. Set **Model -> Custom LLM** and paste `https://<tunnel>/webhook/vapi-faq` as the URL. Set the **First Message** so Ava speaks first (see `labs_local_n8n/lab5/ava-assistant-prompt.md`). Save, then copy the **assistant ID**.
...
10. Serve the site: double-click `labs_local_n8n/lab5/website/start.command` (macOS) or `start.bat` (Windows) - or `cd labs_local_n8n/lab5/website` then `python3 -m http.server 8096` (macOS) / `python -m http.server 8096` (Windows). Open `http://localhost:8096`, click **⚙ Settings** and paste your public key and assistant ID. Nothing is hardcoded: the values live in your browser, so every learner drives the same page with their own assistant.
...
![Lab 4.8 - the Vapi custom-LLM flow: webhook -> AI Agent (Ollama) -> OpenAI-shaped response.](courseware/screenshots/lab5-vapi-flow.png)

*Lab 4.8 - the Vapi custom-LLM flow: webhook -> AI Agent (Ollama) -> OpenAI-shaped response.*

![Lab 4.8 - HomeMart: the FAQ voice agent front end, built on the Vapi Web SDK.](courseware/screenshots/lab5-vapi-site.png)

*Lab 4.8 - HomeMart: the FAQ voice agent front end, built on the Vapi Web SDK.*

![Lab 4.8 - Settings: the learner's own Vapi PUBLIC key and assistant ID. The private key never touches the browser.](courseware/screenshots/lab5-vapi-settings.png)
...
1. Start the studio. The app lives in `labs_local_n8n/lab6/` - nothing to clone. macOS/Linux: `cd lab6` then `./setup.sh` (add `--musetalk` to also pull the ~3.5 GB weights). Windows: follow the PowerShell block in `labs_local_n8n/lab6/README.md`. Open `http://localhost:8137`.
...
4. Paste it into `labs_local_n8n/lab6/lipsyncdemo/.env` as `HEYGEN_API_KEY=...` and **restart the app** - the key is only read at startup. The HeyGen renderer stops being greyed out.
...
![Lab 5.1 - Digital Human Studio (http://localhost:8137): face detected, script loaded, and the avatar speaking in the live preview.](courseware/screenshots/lab6-lipsync-studio.png)

*Lab 5.1 - Digital Human Studio (http://localhost:8137): face detected, script loaded, and the avatar speaking in the live preview.*

![Lab 5.1 - the controls that matter: Draft with Ollama (gemma4), the voice engine, and the LIP SYNC renderer picker.](courseware/screenshots/lab6-lipsync-engines.png)
...
1. Import `labs_local_n8n/lab7/heygen-news-avatar-flow.json`.
...
![Lab 5.3 - the HeyGen news studio: gemma4's script in the teleprompter and an honest render progress bar.](courseware/screenshots/lab7-heygen-site.png)
...
1. Import `labs_local_n8n/lab10/veo3-video-flow.json` and publish it.
2. Create the Gemini API credential in n8n and select it on the Veo nodes.
3. Start the website with `labs_local_n8n/lab10/website/start.command` (Mac) or `start.bat` (Windows), then open `http://localhost:8098`.
...
![Lab 5.4 - the Veo studio: one prompt in, gemma4 writes the shot script, Veo 3.1 renders the clip.](courseware/screenshots/lab10-veo-site.png)
...
1. Open `labs_local_n8n/lab7-opensource/README.md` and review the service architecture.
2. Start the local render service using the provided start script for your platform.
3. Import `labs_local_n8n/lab7-opensource/os-news-avatar-flow.json` into n8n.
...
1. Import `labs_local_n8n/lab8/avatar-chat-flow.json` and publish it.
2. Start the website with `labs_local_n8n/lab8/website/start.command` (Mac) or `start.bat` (Windows), then open `http://localhost:8100` in Chrome or Edge.
...
![Lab 6.2 - Aria, rendered in the browser: speech in, gemma4 reply, and the mouth drawn live. The latency is printed, not claimed.](courseware/screenshots/lab8-interactive-avatar.png)
...
1. Import `labs_local_n8n/lab9/liveavatar-session-flow.json` and publish it.
2. Create the LiveAvatar API credential in n8n as a header credential and select it on both HTTP nodes.
3. Start the website with `labs_local_n8n/lab9/website/start.command` (Mac) or `start.bat` (Windows), then open `http://localhost:8099`.
...
![Lab 6.3 - the LiveAvatar embed: n8n mints a short-lived session URL, so the API key never reaches the browser.](courseware/screenshots/lab9-liveavatar-site.png)
