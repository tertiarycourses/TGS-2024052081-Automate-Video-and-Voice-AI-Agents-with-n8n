...
//
// The page defaults to the local lab0 n8n, so it works as soon as the
...
const DEFAULT_WEBHOOK_URL = "http://localhost:5678/webhook/elevenlabs-web-call";
...
async function renderSetupBanner() {
  ...
  banner.innerHTML = reachable
    ? ""
    : `⚙️ Cannot reach n8n at <strong>${url}</strong> — start the lab0 stack (<code>docker compose up -d</code>) and make sure the ` +
