...
```text
Click "Book by Voice"
  -> browser POSTs to n8n            (http://localhost:5678/webhook/elevenlabs-web-call)
...
1. **Import both flows** into your local n8n (the lab0 Docker stack) and set them **Active**.
...
**Your n8n is on `localhost`, and ElevenLabs' servers cannot reach it.** The tool webhooks must be
public, so run a tunnel:

```bash
ngrok http 5678
```

Use `https://<id>.ngrok-free.app/webhook/check-availability` (and `/book-appointment`) as the tool URLs
in the ElevenLabs agent. The web-call webhook does **not** need the tunnel — your own browser calls that one.
