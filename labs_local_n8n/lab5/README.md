...
**Vapi**, and answering from **Ollama `gemma4:latest`** running in your local n8n (the lab0 Docker stack).
...
└── vapi-faq-flow.json       # n8n Custom LLM: webhook -> agent (Ollama `gemma4:latest`) -> OpenAI-shaped response
```

## Setup

1. **Import `vapi-faq-flow.json`** into your local n8n (the lab0 Docker stack) and set it **Active**.
2. The *FAQ Agent* node uses the **Ollama local** (`http://host.docker.internal:11434`) credential.
3. Vapi's servers call your n8n as a **Custom LLM**, so `/webhook/vapi-faq` must be public too:

```bash
ngrok http 5678
```

Put `https://<id>.ngrok-free.app/webhook/vapi-faq` in the Vapi assistant's Custom LLM URL.
