...
> 🪟 **Windows blocks downloaded files.** A ZIP downloaded from GitHub carries the
> *Mark-of-the-Web*, so Windows flags everything inside as blocked. **Before extracting:
> right-click the ZIP → Properties → tick "Unblock" → OK.** If you already extracted it and
> SmartScreen warns on `start.bat`, click **More info → Run anyway** — the launcher then
> clears the flag on the rest of the lab's files itself. (`git clone` avoids all of this.)
...
- [PowerPoint deck](courseware/Automate-Video-and-Voice-AI-Agents-with-n8n-v4.0.pptx)
...
The **v4.0** courseware is generated by
[`scripts/generate_agentic_courseware.py`](scripts/generate_agentic_courseware.py) and is fully
aligned to the three-topic structure above: **2 training days** (Day 1 morning — Chatbot, Day 1
afternoon — Voice Agent, Day 2 — Video Agent + assessment), delivering the WSQ skill
**Artificial Intelligence Application (AER-TEM-4026-1.1)** with the v4.0 WA (SAQ) + Case Study
assessment set.
